...
RSpec.describe Authsnitch::Summarizer do
  ...
  describe '#summarize' do
    ...
    it 'sets title for notification' do
      result = summarizer.summarize(
        detection_result: detection_result,
        pr_info: pr_info,
        keywords_detected: [],
      ...
      expect(result[:title]).to eq('AuthSnitch - Authentication Changes Detected')
    end

    it 'sets title for non-notification' do
      result = summarizer.summarize(
        detection_result: detection_result,
        pr_info: pr_info,
        keywords_detected: [],
      ...
      expect(result[:title]).to eq('AuthSnitch - Review Summary')
    end
